\section{Da Persuasão ao Controle da Resposta: Refinamento, Variação entre Modelos e Prefill}

\label{sec:jonas}

A discussão dos PAPs mostra como estruturar a persuasão como vetor de ataque. Os estudos desta seção ampliam essa análise ao investigar o refinamento iterativo dos prompts, a variação de suscetibilidade entre modelos e a intervenção no início da resposta. Esse percurso conecta a construção do argumento às condições em que o modelo gera sua saída.

Um caso ocorrido no Reino Unido ilustra bem esse risco: um cliente frustrado convenceu o chatbot de atendimento da transportadora DPD a "ignorar as regras", xingar a própria empresa e escrever um haiku \footnote{Haiku: forma de poema curto de origem japonesa, tradicionalmente estruturado em três versos com contagem fixa de sílabas (5-7-5). HOUAISS, Antônio; VILLAR, Mauro de Salles. \textbf{Dicionário Houaiss da língua portuguesa}. Rio de Janeiro: Objetiva, 2001. Verbete: haicai.} insultando o serviço prestado. Diante do episódio, a DPD desativou o componente de IA no mesmo dia \cite{theregister2024dpd}.

O episódio ilustra consequências práticas de comportamentos inadequados, mas não constitui uma avaliação dos métodos analisados. Para relacionar os estudos experimentais, distinguimos duas questões: como adaptar a entrada persuasiva ao modelo e como a possibilidade de controlar o prefixo da resposta altera a superfície de ataque \cite{noughabi2025uncoveringpersuasivefingerprintllms,ke2025iterativepromptingpersuasionskills,li2025prefillleveljailbreakblackboxrisk}.

\subsection{O Uso de Técnicas de Persuasão para Enganar os Modelos}

Ke et al. retomam o uso de técnicas de persuasão na reformulação de pedidos e investigam seu refinamento ao longo de rodadas \cite{ke2025iterativepromptingpersuasionskills}. Em relação aos PAPs, o interesse desloca-se da organização das estratégias persuasivas para o uso do desempenho dos ataques como retorno para novos ajustes. Há também uma aproximação conceitual com o AutoDAN: ambos exploram a revisão de candidatos, embora a reformulação persuasiva e a busca genética constituam mecanismos distintos.

No sistema descrito por Ke et al., um modelo atacante utiliza cinco táticas de persuasão selecionadas a partir do trabalho de Zeng et al. \cite{ke2025iterativepromptingpersuasionskills,zeng2024johnny}:
...
O refinamento avaliado por Ke et al. permite observar ganhos ao longo das rodadas, mas não implica que todos os modelos respondam igualmente aos mesmos argumentos. Essa variação é o foco do trabalho de Noughabi et al., que caracteriza uma ``assinatura persuasiva'' (\textit{persuasive fingerprint}) a partir da suscetibilidade aos princípios de influência social de Cialdini \cite{noughabi2025uncoveringpersuasivefingerprintllms}. Enquanto os PAPs organizam técnicas de construção de mensagens e Ke et al. investigam seu ajuste iterativo, esse estudo examina diferenças entre os modelos avaliados. Os perfis relatados incluem:
...
Os estudos de persuasão discutidos até aqui intervêm na entrada para aumentar a chance de cooperação do modelo. O ataque por \textit{prefill} altera o ponto de intervenção ao permitir que parte da resposta seja fornecida antecipadamente. A relação com o GCG, apresentado no início da análise, torna-se especialmente clara: ambos procuram favorecer uma continuação afirmativa, mas o GCG otimiza o prompt para induzir esse início, enquanto o prefill atua diretamente sobre o prefixo de saída, quando a interface oferece esse recurso \cite{zou2023universaltransferableadversarialattacks,li2025prefillleveljailbreakblackboxrisk}. Essa diferença acrescenta as condições de acesso à comparação entre métodos.

Li et al. (2025) \cite{li2025prefillleveljailbreakblackboxrisk} descobriram que os canais de programação (APIs) de grandes empresas como Anthropic (Claude) e DeepSeek possuem uma função legítima chamada \textbf{preenchimento prévio de resposta (\textit{user-controlled response prefilling})} \cite{li2025prefillleveljailbreakblackboxrisk}. Essa função serve originalmente para ajudar o sistema a organizar as saídas (como exigir que a resposta comece com um código ou no formato JSON) \cite{li2025prefillleveljailbreakblackboxrisk}. A Figura \ref{fig:prefill} ilustra uma comparação de ataques de jailbreak em nível de prompt versus nível de pré-preenchimento.
...
\subsection{Complementaridade entre Ataques e Implicações para as Defesas}

A distinção entre reformular a entrada e controlar o prefixo da saída ajuda a compreender por que esses mecanismos podem ser combinados. O estudo de Li et al. apresenta evidências dessa complementaridade ao avaliar ataques em conjunto com prefill \cite{li2025prefillleveljailbreakblackboxrisk}. Quando as técnicas de forçar o início da resposta (\textit{prefill}) são combinadas com ataques tradicionais de persuasão ou otimização de texto (como os métodos PAIR ou ReNeLLM), as taxas de sucesso do ataque aumentam entre 10 e 15 pontos percentuais \cite{li2025prefillleveljailbreakblackboxrisk}. O método PAIR, por exemplo, apresentava isoladamente uma taxa de sucesso de 48,2\% contra o modelo Gemini-2.5-Flash; ao adicionar a técnica de preenchimento prévio, essa taxa saltou para 90,2\% \cite{li2025prefillleveljailbreakblackboxrisk}.
...
Essa defesa retoma uma questão que atravessa os trabalhos analisados: o risco depende da relação entre a forma da mensagem, seu significado e a continuação solicitada ao modelo. O AutoDAN destaca a limitação de associar suspeita à baixa fluência; o FlipAttack explora a recuperação do significado de entradas transformadas; os estudos de persuasão examinam o enquadramento argumentativo; e o prefill acrescenta o controle do início da saída. Essa leitura conjunta sugere avaliar os diferentes pontos de intervenção e suas possíveis interações. A evidência de combinações específicas, entretanto, não demonstra que qualquer par de técnicas produza os mesmos ganhos, nem permite ordenar métodos avaliados com modelos, dados e critérios distintos.
